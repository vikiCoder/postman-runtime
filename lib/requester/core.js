    ...
    getRequestHeaders: function (request) {
        ...
        if (request.headers) {
            request.forEachHeader(function (header) {
                ...
                // @note: Node will set header name to lowercase as well.
                // This makes sure we don't create multiple case sensitive headers,
                // Since, `setHeader` will replace the value if header already exists.
                // Reference: https://github.com/nodejs/node/blob/v10.14.2/lib/_http_outgoing.js#L468
                var key = header.key.toLowerCase(),
                    value = header.value;

                // if header already exists, we need to accumulate it in an array
                if (headers[key]) {
